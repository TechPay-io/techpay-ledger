...
// PHOTON_CHANGE_INTERNAL is index of internal Photon chain (internal change in terms of BIP44 specs)
static const uint32_t PHOTON_CHANGE_INTERNAL = 1;

// PHOTON_CHANGE_EXTERNAL is index of public Photon chain (external change in terms of BIP44 specs)
static const uint32_t PHOTON_CHANGE_EXTERNAL = 0;
...
// bip44_hasValidTechPayPrefix implements test for valid BIP44 prefix in the BIP44 path
// for TechPay address derivation.
bool bip44_hasValidTechPayPrefix(const bip44_path_t *path) {
#define CHECK(cond) if (!(cond)) return false
    ...
    // the coin type has to be valid TechPay coin type and should be hardened
    CHECK(path->path[BIP44_I_COIN_TYPE] == (TECHPAY_COIN_TYPE | HARDENED_BIP32));
    return true;
#undef CHECK
}
...
bool bip44_hasValidChangeType(const bip44_path_t *path) {
    ...
    return (type == PHOTON_CHANGE_EXTERNAL) || (type == PHOTON_CHANGE_INTERNAL);
}
