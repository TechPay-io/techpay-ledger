...
static void handleSignTxFinalize(uint8_t p2, uint8_t *wireBuffer MARK_UNUSED, size_t wireSize) {
    ...
    // We sign only TechPay chain messages to mitigate possible replay attacks.
...
static void runSignTransactionUIStep() {
    ...
    switch (ctx->uiStep) {
        ...
        case UI_STEP_TX_AMOUNT: {
            ...
            // create formatted address buffer and format for display
            char valueStr[40];
            txGetFormattedAmount(&ctx->tx.value, WEI_TO_TPC_DECIMALS, valueStr, sizeof(valueStr));
        ...
        case UI_STEP_TX_FEE: {
            ...
            // create formatted address buffer and format for display
            char valueStr[40];
            txGetFormattedFee(&ctx->tx, WEI_TO_TPC_DECIMALS, valueStr, sizeof(valueStr));
